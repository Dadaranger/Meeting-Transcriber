...
def test_recording_lifecycle_tracks_timestamps_and_revisions() -> None:
    ...
    session = session.transition(
        SessionState.PAUSED, at=record_time + timedelta(minutes=1)
    )
    session = session.transition(
        SessionState.RECORDING, at=record_time + timedelta(minutes=2)
    )
...
def test_processing_can_retry_without_losing_recording_stop_time() -> None:
    ...
    session = session.transition(
        SessionState.RECORDING, at=START + timedelta(seconds=2)
    )
    session = session.transition(SessionState.RECORDED, at=START + timedelta(minutes=2))
    stopped_at = session.stopped_at

    session = session.transition(
        SessionState.PROCESSING, at=START + timedelta(minutes=3)
    )
